...
| Memory management (`lmkd`) | ❌ | Android cannot reclaim inside the container. `lmkd` runs and has minfree levels, but there is no `/dev/memcg`, `/sys/fs/cgroup/memory/lxc.payload.waydroid` is mounted **read-only**, and this 3.18 kernel has no PSI (`/proc/pressure`), so nothing ever wakes it — zero kills observed at 9 MB free. The host's `lowmemorykiller` does not step in either: it weighs page cache, which stays high while anonymous memory thrashes to zram. The device degrades by thrashing, which shows up as touch freezing for tens of seconds → [rca](rca/waydroid-touch-anr-thrashing.md) |
