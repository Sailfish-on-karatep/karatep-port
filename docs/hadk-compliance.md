...
| 13 | Proximity sensor in suspend | ⚠️ the sensor itself is **now verified** — it is read on demand during calls and drives display blanking correctly (see #16). The checklist's actual concern, behaviour across a suspend/resume cycle, remains untested and is blocked behind #9 |
...
| 16 | Proximity blanking during a call | ✅ **verified 2026-08-13 on a live call.** mce powers the sensor on demand (`UNDEF -> OPEN` at call start, `OPEN -> UNDEF` after it ends) and ran four clean cover/uncover cycles, each a full `POWER_DOWN -> MDSS_EVENT_BLANK -> OFF` / `POWER_UP -> UNBLANK -> ON` round trip, unblank costing a consistent 150-160 ms |
