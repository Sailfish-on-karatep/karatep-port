...
| VoLTE | ❌ | **Does not register, root-caused on hardware.** ofono *reports* it registered; that is a false positive with two causes. (1) The vendor RIL wires `requestRegistrationChange` — the only IMS enable call `ofono-binder-plugin-ext-qti` makes — to QMI IMSS *set IMS test mode*, and this production modem refuses it: both it and `getImsRegistrationState` return `RadioError::GENERIC_FAILURE` (2), captured in the binder hexdump. So nothing ever asks the modem to register, and qcril's IMSA reports `valid 0` everywhere — no indication, rather than a negative one. (2) ext-qti's `qti_ims_reg_status_response()` never checks that error code, parses the zeroed payload behind it, and `REGISTERED == 0`, so a refusal decodes as success; `VoiceCapable` and `SmsCapable` are hardcoded constants too. Every call therefore CSFBs to 2G. The migration to `ofono-binder-plugin` + `ofono-binder-plugin-ext-qti` is done and is *not* the problem — it is a prerequisite. What is missing is `setServiceStatus` (transaction 9) and possibly `setConfig` (12, `CONFIG_ITEM_VOLTE_USER_OPT_IN_STATUS = 33`), the calls Android's `ims.apk` actually uses; ext-qti implements neither, and the porters archive has **zero** hits for any of them, so there is no prior art → [rca](docs/rca/volte-registration-change-is-test-mode.md) |
