...
| GPS | ❌ | stack works and sessions reach the hardware, but the receiver reports no satellites in view: no `$GPGSV`, `mEngineOn: 0`, only coarse network seeds. Assistance is separately broken (no XTRA, no aGPS). → [details](docs/porting-notes.md#gps-does-not-acquire) |
