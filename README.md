...
| VoLTE | ❌ | **The modem does not offer IMS at all — root-caused down to the carrier config.** Both ofono-side defects are fixed in our fork of `ofono-binder-plugin-ext-qti` (`hybris-18.1`): the plugin no longer reports a refused `getImsRegistrationState` as a successful registration (`Registered` now reads `false`, truthfully), and `setServiceStatus` — the call Android's `ims.apk` uses, not `requestRegistrationChange`, which this RIL wires to QMI IMSS *set IMS test mode* — is implemented and accepted by qcril. It still fails, because qcril's IMSA/IMSS/IMSP QMI clients all time out at init: `/sys/kernel/debug/msm_ipc_router/dump_servers` shows the modem offering every service qcril needs **except** `0x12`/`0x1f`/`0x20`/`0x21`. Separately, no MBN carrier config was ever loaded — `/vendor/bin/init.qcom.sh` copies from a `modem_pr/mcfg/configs` tree this firmware does not have, then sets `mbn_copy_completed=1` regardless; staging the eight flat `.mbn` files by hand plus `persist.vendor.radio.sw_mbn_update=1` makes selection and activation work (`sw_mbn_loaded=1`), but the generic `row.mbn` it picks leaves IMS off. The reason is now exact, and it is two bytes: no config on this device matches **MCC 404** — India is 404, Jio being the exception at 405 — so a BSNL SIM falls through to the `wild` catch-all `ROW_Generic_3GPP` (`row.mbn`), and decoding that config shows it sets `ims/IMS_enable = 0` and `modem/mmode/voice_domain_pref = CsVoiceOnly`. Every non-Jio SIM on this device gets a modem with IMS switched off in NV, which is why the IMS QMI services are absent. The modem firmware itself has the full VoLTE stack, so this is configuration, not capability. The configs are unsigned (three SHA-256 hashes, no cert chain), so a patched `row.mbn` with `IMS_enable = 1` and `voice_domain_pref = ImsPsVoicePreferred` was built with `scripts/patch-mbn-ims.sh` — no carrier impersonation needed — and **the modem has now loaded, selected and activated it**, surviving reboots. Two further qcril gates had to be opened to get there: `persist.vendor.radio.sw_mbn_volte`/`sw_mbn_openmkt` ship empty, and with them empty no config is ever selected; and a config whose MCFG version is unchanged is silently skipped, so the patch bumps the minor version. The modem now publishes three of the four IMS QMI services (`0x20` still absent) and IMSA returns real data (`IMS registered valid 1`) instead of nothing. Adding an IMS data profile (APN `ims`) was not sufficient, but importing the generic IMS parameter set was: the modem now accepts `setServiceStatus` (`IMS voice service enabled`) with no `RIL_E_MODEM_ERR` anywhere. Bisected in three rounds to **one item** — `nv/item_files/ims/qipcall_config_items`, the QIPCALL blob carrying `VolteDisabled = 0`; without it the modem has no QIPCALL configuration and `set_qipcall_config` fails. It contains no carrier identity. The network is confirmed good — the same SIM registers IMS and calls immediately in another handset at the same location — so what is left is handset-side. The fork now also sends `setConfig` (transaction 12, recovered from `ims.apk`), the provisioning half that Android's framework writes when the VoLTE switch is flipped and that BSNL users force open on Xiaomi with `*#*#86583#*#*`: `VLT_SETTING_ENABLED` is **accepted**, `VOLTE_USER_OPT_IN_STATUS` returns `CONFIG_WRITE_FAILED` because it routes through the IMS presence service this modem does not publish. Every request now succeeds and IMS still never registers. The IMS PDN turned out to be ofono's to bring up, not the modem's: `/ril_0/context3` (`Type: ims`) sits inactive, and activating it by hand gives a real bearer on `rmnet_data1` using `PROFILE_ID = [2]`, the data profile we imported. `qcril_data_set_apn_types` fails with error 57 there, but reading `ofono-binder-plugin` 1.1.25 shows it already maps ims contexts to `RADIO_DATA_PROFILE_IMS` and sends `supportedApnTypesBitmap` on the sub-1.4 HIDL path, with `use_data_profiles` defaulting to on — so that failure is qcril-to-modem, not a missing field of ours, and the plugin should not be forked for it. Newly visible: qcril's **QMI LTE client times out at every `rild` start** (`QMI_TIMEOUT_ERR`) while every other core client returns 0. Also fixed on the way: the imported `qp_ims_reg_config` carried `RegOnMode = OnCall`, so the modem would only ever have registered while placing a call — `scripts/mbn-set-regonmode.sh` sets it to `PowerOn` → [rca](docs/rca/volte-registration-change-is-test-mode.md) |
