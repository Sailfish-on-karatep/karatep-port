...
| GPS | ⚠️ | tracks satellites, does not fix. Forcing STANDALONE (`GEOCLUE_HYBRIS_POSITION_MODE`) turns the engine on — 16 SVs in view across GPS and GLONASS, XTRA and NTP injected — but C/N0 peaks at 31 dBHz with only 1–2 satellites audible, short of the four needed. Requesting MS_BASED, the upstream default, stalls the engine outright. → [details](docs/porting-notes.md#gps-tracks-satellites-does-not-fix) |
