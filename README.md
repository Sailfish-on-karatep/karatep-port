...
| Cellular — mobile data | ❌ | **Broken, and root-caused.** Sailfish disables Android's `netd`, but this vendor's `netmgrd` is a client of the Netd *HAL* and waits forever for `android.system.net.netd@1.1::INetd`, so `dsi_netctrl` never hands `rild` a handle and every `SETUP_DATA_CALL` dies at `unable to get dsi hndl` → ofono `Unexpected data call status 4100`. Fix (`external/stub_netd`) is built and pinned; hardware verification pending → [rca](docs/rca/mobile-data-no-dsi-handle.md) |
...
| VoLTE | ⚠️ | **Not Jolla-proprietary — the old note was wrong.** The hardware exposes `vendor.qti.hardware.radio.ims@1.0::IImsRadio/imsradio0` and `/imsradio1`, both registered and live, and the four vendor IMS daemons run cleanly. What is missing is the ofono side: the image ships the legacy grilio stack (`ofono-ril-plugin` + `ofono-ril-binder-plugin`), and IMS extensions load only into `ofono-binder-plugin` via `libofonobinderpluginext`, so no `org.ofono.IpMultimediaSystem` interface exists. The migration is a supported swap (`ofono-binder-plugin` Obsoletes/Conflicts both ril packages) and every package is prebuilt for aarch64 in repos the image already uses; `ofono-binder-plugin-ext-qti` then supplies VoLTE. Registration looks achievable; a fully clean call is not guaranteed on `IRadio@1.1` |
