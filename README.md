...
| VoLTE | ❌ | **Still does not register, but the blocker has moved out of ofono.** Two ofono-side defects are fixed in our fork of `ofono-binder-plugin-ext-qti` (`hybris-18.1`). (1) The plugin reported IMS registered when it was not: this HAL refuses `getImsRegistrationState` with `GENERIC_FAILURE` yet still returns a *zeroed* `RegistrationInfo`, and `REGISTERED == 0`, so an unchecked `info->state` turned every refusal into a claim of success. Now checked — `Registered` reads `false`, which is the truth. (2) `requestRegistrationChange` is not the IMS enable call on this RIL — qcril wires it to QMI IMSS *set IMS test mode* — so `setServiceStatus` (transaction 9) is now implemented and sent alongside it. Getting qcril to accept it needed a non-empty `accTechStatus` naming `RADIO_TECH_LTE`, and a fix to `QtiRadioServiceStatusInfo`, whose `bool` fields were declared 4 bytes wide and made the struct 72 bytes against the interface's 64. qcril now parses it correctly (`has_calltype 1`, `has_status 2`) — and fails one layer down at `qmi send async res 1`, the *same* failure every other IMSS operation hits, while NAS and UIM work fine. So qcril has no usable QMI IMS Settings client, and that is the remaining blocker → [rca](docs/rca/volte-registration-change-is-test-mode.md) |
